...
@objc open class BRWebViewController : UIViewController, WKNavigationDelegate, BRWebSocketClient, WKScriptMessageHandler {
    ...
    var indexUrl: URL {
        switch mountPoint {
            ...
            case "/buy_coinbase":
                return URL(string: "https://api.Litewallet.org/buy")!
            case "/support":
                return URL(string: "https://api.Litewallet.org/support")!
            case "/ea":
                return URL(string: "https://api.Litewallet.org/ea")!
            default:
    ...
    private func getSimplexParams(appInstallDate: Date?, walletAddress: String?, currencyCode: String?, uuid: String?) -> String {
        ...
        return "https://buy.Litewallet.org/?address=\(walletAddress)&code=\(currencyCode)&idate=\(timestamp)&uid=\(uuid)"
    }
