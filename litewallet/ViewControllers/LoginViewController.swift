...
class LoginViewController: UIViewController, Subscriber, Trackable {
    ...
    private let pinPadViewController = PinPadViewController(
        style: .clear, keyboardType: .pinPad, maxDigits: 0)
    ...
    private let deprecationMessageLabel = UILabel(font: .barlowSemiBold(size: 16), color: .white)
    private let deprecationMessageContainer = UIView()
    ...
    private let biometricsButton: UIButton = {
        ...
        button.setImage(
            LAContext.biometricType() == .face
                ? #imageLiteral(resourceName: "FaceId") : #imageLiteral(resourceName: "TouchId"),
            for: .normal)
        ...
        button.accessibilityLabel =
            LAContext.biometricType() == .face
            ? S.UnlockScreen.faceIdText.localize() : S.UnlockScreen.touchIdText.localize()
        return button
    }()
    ...
    override func viewDidLoad() {
        ...
        disabledView.didTapReset = { [weak self] in
            ...
            let recover = EnterPhraseViewController(
                store: store, walletManager: walletManager,
                reason: .validateForResettingPin { phrase in
                    let updatePin = UpdatePinViewController(
                        store: store, walletManager: walletManager, type: .creationWithPhrase,
                        showsBackButton: false, phrase: phrase)
    ...
    private func addPinView() {
        ...
        logo.constrain([
            logo.topAnchor.constraint(equalTo: view.centerYAnchor, constant: -140),
        ...
        pinView.constrain([
            pinView.centerYAnchor.constraint(
                equalTo: pinPadViewController.view.topAnchor, constant: -40),
    ...
    private func addSubviews() {
        ...
        view.addSubview(deprecationMessageContainer)
        deprecationMessageContainer.addSubview(deprecationMessageLabel)
    ...
    private func addConstraints() {
        ...
        if walletManager != nil {
            addChildViewController(
                ...
                layout: {
                    pinPadBottom = pinPadViewController.view.bottomAnchor.constraint(
                        equalTo: view.bottomAnchor, constant: -120)
                    pinPadViewController.view.constrain([
                        pinPadViewController.view.leadingAnchor.constraint(
                            equalTo: view.leadingAnchor),
                        pinPadViewController.view.trailingAnchor.constraint(
                            equalTo: view.trailingAnchor),
                        pinPadBottom,
                        pinPadViewController.view.heightAnchor.constraint(
                            equalToConstant: pinPadViewController.height),
                    ])
                })
        }
        ...
        if walletManager != nil {
            pinPadBackground.constrain([
                pinPadBackground.leadingAnchor.constraint(
                    equalTo: pinPadViewController.view.leadingAnchor),
                pinPadBackground.trailingAnchor.constraint(
                    equalTo: pinPadViewController.view.trailingAnchor),
                pinPadBackground.topAnchor.constraint(equalTo: pinPadViewController.view.topAnchor),
                pinPadBackground.bottomAnchor.constraint(
                    equalTo: pinPadViewController.view.bottomAnchor),
            ])
        } else {
        ...
        // Configure deprecation message container
        deprecationMessageContainer.backgroundColor = UIColor.black.withAlphaComponent(0.7)
        deprecationMessageContainer.layer.cornerRadius = 12
        deprecationMessageContainer.layer.masksToBounds = true
        deprecationMessageContainer.layer.borderWidth = 1.5
        deprecationMessageContainer.layer.borderColor = UIColor.systemYellow.cgColor
        
        // Configure deprecation message label
        deprecationMessageLabel.text =
            "⚠️ Litewallet is no longer being maintained, please upgrade to Nexus Wallet for Litecoin."
        deprecationMessageLabel.textAlignment = .center
        deprecationMessageLabel.numberOfLines = 0
        deprecationMessageLabel.textColor = .systemYellow
        deprecationMessageLabel.backgroundColor = .clear
        
        // Add padding to the label text with better formatting
        let attributedText = NSMutableAttributedString(string: deprecationMessageLabel.text ?? "")
        let paragraphStyle = NSMutableParagraphStyle()
        paragraphStyle.alignment = .center
        paragraphStyle.lineSpacing = 4
        attributedText.addAttribute(.paragraphStyle, value: paragraphStyle, range: NSRange(location: 0, length: attributedText.length))
        deprecationMessageLabel.attributedText = attributedText

        // Add constraints for deprecation message container
        deprecationMessageContainer.constrain([
            deprecationMessageContainer.topAnchor.constraint(equalTo: logo.bottomAnchor, constant: 20),
            deprecationMessageContainer.leadingAnchor.constraint(
                equalTo: view.leadingAnchor, constant: 24),
            deprecationMessageContainer.trailingAnchor.constraint(
                equalTo: view.trailingAnchor, constant: -24),
        ])
        
        // Add constraints for deprecation message label inside container
        deprecationMessageLabel.constrain([
            deprecationMessageLabel.topAnchor.constraint(equalTo: deprecationMessageContainer.topAnchor, constant: 12),
            deprecationMessageLabel.leadingAnchor.constraint(equalTo: deprecationMessageContainer.leadingAnchor, constant: 16),
            deprecationMessageLabel.trailingAnchor.constraint(equalTo: deprecationMessageContainer.trailingAnchor, constant: -16),
            deprecationMessageLabel.bottomAnchor.constraint(equalTo: deprecationMessageContainer.bottomAnchor, constant: -12),
        ])
    }
    ...
    private func addBiometricsButton() {
        ...
        biometricsButton.constrain([
            ...
            biometricsButton.leadingAnchor.constraint(
                equalTo: view.leadingAnchor, constant: C.padding[2]),
            biometricsButton.topAnchor.constraint(
                equalTo: view.topAnchor, constant: headerHeight + C.padding[2]),
        ])
    }
    ...
    private func addShowAddressButton() {
        ...
        showLTCAddressButton.constrain([
            ...
            showLTCAddressButton.trailingAnchor.constraint(
                equalTo: view.trailingAnchor, constant: -C.padding[2]),
            showLTCAddressButton.topAnchor.constraint(
                equalTo: view.topAnchor, constant: headerHeight + C.padding[2]),
        ])
    }
    ...
    private func addPinPadCallback() {
        pinPadViewController.ouputDidUpdate = { [weak self] pin in
            ...
            self?.pinPadViewController.isAppendingDisabled =
                attemptLength < myself.store.state.pinLength ? false : true
    ...
    private func authenticationSucceded() {
        ...
        UIView.spring(
            ...
            animations: {
                ...
                self.deprecationMessageContainer.alpha = 0.0
    ...
    private var shouldUseBiometrics: Bool {
        ...
        return LAContext.canUseBiometrics && !walletManager.pinLoginRequired
            && store.state.isBiometricsEnabled
    }

    @objc func biometricsTapped() {
        guard !isWalletDisabled else { return }
        walletManager?.authenticate(
            biometricsPrompt: S.UnlockScreen.touchIdPrompt.localize(),
        ...
        if let disabledUntil = walletManager?.walletDisabledUntil {
            ...
            if disabledUntil > now {
                ...
                df.setLocalizedDateFormatFromTemplate(
                    unlockInterval > C.secondsInDay ? "h:mm:ss a MMM d, yyy" : "h:mm:ss a")

                disabledView.setTimeLabel(
                    string: String(
                        format: S.UnlockScreen.disabled.localize(),
                        df.string(from: disabledUntilDate)))

                pinPadViewController.view.isUserInteractionEnabled = false
                unlockTimer?.invalidate()
                unlockTimer = Timer.scheduledTimer(
                    timeInterval: unlockInterval, target: self,
                    selector: #selector(LoginViewController.unlock), userInfo: nil, repeats: false)
