import Foundation
import SwiftUI

extension Color
{
	static var liteWalletBlue: Color
	{ // 345D9D
		return Color(UIColor.liteWalletBlue)
	}

	static var liteWalletDarkBlue: Color
	{ // 163e7f
		return Color(UIColor.liteWalletDarkBlue)
	}

	static var litewalletLightGray: Color
	{ // F8F8F8
		return Color(UIColor.litewalletLightGray)
	}

	static var litecoinGray: Color
	{ // F1F1F1
		return Color(UIColor.litecoinGray)
	}

	static var litecoinSilver: Color
	{ // A6A9AA
		return Color(UIColor.litecoinSilver)
	}

	static var litecoinDarkSilver: Color
	{ // 4D4D4E
		return Color(UIColor.litecoinDarkSilver)
	}
}
