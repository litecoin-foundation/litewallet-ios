...
private class BRBrowserViewControllerInternal: UIViewController, WKNavigationDelegate {
    ...
    open override func viewDidLoad() {
        ...
        view.addConstraints(NSLayoutConstraint.constraints(
            ...
            views: ["progressView": progressView, "topGuide": topLayoutGuide]
        ))
        ...
        view.addConstraints(NSLayoutConstraint.constraints(
            ...
            views: ["webView": webView, "toolbarContainer": toolbarContainerView, "topGuide": topLayoutGuide]
        ))
    }
    ...
    func showLoading(_ isLoading: Bool) {
        ...
        if isLoading {
            toolbarView.items = [backButtonItem, forwardButtonItem, flexibleSpace, stopButtonItem]
        } else {
            toolbarView.items = [backButtonItem, forwardButtonItem, flexibleSpace, refreshButtonItem]
        }
    }
    ...
    open func webView(_: WKWebView, didCommit navigation: WKNavigation?) {
    ...
    open func webViewWebContentProcessDidTerminate(_: WKWebView) {
    ...
    open func webView(_: WKWebView, didFailProvisionalNavigation navigation: WKNavigation?,
    ...
    open func webView(_: WKWebView, didStartProvisionalNavigation navigation: WKNavigation?) {
        if let navigation = navigation {
...
open class BRBrowserViewController: UINavigationController {
    ...
    @objc private func done(_: UIControl?) {
    ...
    open override func dismiss(animated flag: Bool, completion: (() -> Void)? = nil) {
        if presentedViewController == nil, !isClosing { return }
        isClosing = false
        super.dismiss(animated: flag, completion: completion)
    }
}
