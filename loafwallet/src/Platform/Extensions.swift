...
public extension Data
{
	...
	var bzCompressedData: Data?
	{
		...
		var mself = self
		var success = true
		mself.withUnsafeMutableBytes
		{ (selfBuff: UnsafeMutablePointer<Int8>) in
	...
	init?(bzCompressedData data: Data)
	{
		...
		{ (datBuff: UnsafeMutablePointer<Int8>) in
			let outBuff = UnsafeMutablePointer<Int8>.allocate(capacity: Int(BZCompressionBufferSize))
			defer { outBuff.deallocate() }
	...
	var base58: String
	{
		...
		{ (selfBytes: UnsafePointer<UInt8>) -> String in
			let len = BRBase58Encode(nil, 0, selfBytes, self.count)
			var data = Data(count: len)
			return data.withUnsafeMutableBytes
			{ (b: UnsafeMutablePointer<Int8>) in
	...
	var sha1: Data
	{
		...
		{ (bytes: UnsafeMutablePointer<UInt8>) in
			...
			{ (selfBytes: UnsafePointer<UInt8>) in
	...
	{
		return withUnsafeBytes
		{ (ptr: UnsafePointer<UInt256>) -> UInt256 in
			ptr.pointee
		}
	}
